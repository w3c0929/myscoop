...
elseif ($select -eq "3") {
    ...
    foreach ($bkt in $exportData.buckets) {
        ...
        if ($mirrorBuckets.ContainsKey($name)) {
            ...
            $outputLines += "      if ((Test-Path (Join-Path `$scoopPath ""buckets\$name"")) -or (`$existingList -match [regex]::Escape('$name'))) {"
            ...
            $outputLines += "      if (`$LASTEXITCODE -ne 0) {"
            $outputLines += "        if (`$output -match 'already exists') {"
    ...
    foreach ($app in $exportData.apps) {
        ...
        if ($bucket) {
            ...
            $outputLines += "  if (-not ((Test-Path (Join-Path `$scoopPath ""buckets\$bucket"")) -or (`$bucketCheck -match [regex]::Escape('$bucket')))) {"
