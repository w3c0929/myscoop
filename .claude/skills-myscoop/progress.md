...
```
* 91456d4 (HEAD) NInfer 升级 1.0.8→1.0.9：删除远程 v1.0.8 release，发布 v1.0.9（ninfer-ternary-ada-sm89-win64-v1.0.9-protable.zip，digest 与清单一致 0df44c9a）；hash 规范化小写；README 版本列同步
* d6b85c3 (origin/main, origin/HEAD) progress.md 更新提交历史
* 1c3c112 自动更新第三方软件
* 0e39690 progress.md 更新提交历史
* 2a3cb5b CI 自动同步 README 版本：新增 tools/sync_readme_versions.py（第三方表版本列 = 清单 version 原样，幂等，--check 干跑），auto-update.yml 每晚 --all 后自动同步并一并提交；首次落地同步 8 处滞后版本（cmm-plus 1.7.6 / cc-haha 0.6.6 / cinetry 0.8.4 / comfyui 0.37.0 / lertaro 5.7.3 / amcfy-music 1.2.11 / prllama prism-b10709-9a9394a / updist aivault-v0.2.9）
...
* 15bfe95 自动更新第三方软件
* 6614413 新增updist.json AI Vault 是本地优先的 AI 创作者工作台：把技能、提示词、MCP、ComfyUI 工作流、模型与在线平台收进一个本地保险库，并分发到你使用的各个 AI Agent
...
* b912205 progress.md 更新提交历史
* 102ac57 扁平化判据按用户定稿：生成侧只看顶层结构——恰好一个文件夹（无散文件）才扁平化（top_exe 条件移除）；文件夹+文件并列（src+exe）与多文件夹不扁平；运行侧保留"目录含 exe"守卫作安全兜底；单测新增纯数据文件夹场景，7 套全过
* aaf28e6 progress.md 更新提交历史
...
* f945985 qingjian.json 增加 notes：输入法装完未出现时的免重启处理指引（新窗口即时生效/重启 ctfmon/资源管理器/注销）
* 3f6ad46 progress.md 更新提交历史
...
* 175daba 自动更新第三方软件
* 1bf6190 progress.md 更新提交历史
* d8091ff myscoop-update.py 同步修复：NSIS pre_install 模板弃用 {{FILE}} 硬编码，改动态匹配 $dir 内 exe，版本升级免维护
* 7a10f22 progress.md 更新提交历史
...
* 1c6b92f 移除顶层铁律：gzh 已独立为嵌套私人仓库（git init），外层不再需要 gzh 约束
* 9bd49ae progress.md 更新提交历史
* 0448050 gzh 工作流工具脚本入库：封面升级链(v2/v4/cartoon-v4 超采样)、标签补足、end-mark 移除
* 90dad3e progress.md 更新提交历史
...
* 5845a18 progress.md 更新提交历史
* a7e126c 修复：仅发布 prerelease 的仓库(如 Magpie)更新 404——/releases/latest 回退取最新 prerelease；tag 带后缀且与清单同主版本时视为无更新
* 9a8b314 progress.md 更新提交历史
...
* d18821a progress.md 更新提交历史
* 36ba999 myscoop-update.py 顶部用法说明补充直链/下载页/模板新增模式
* 1b4c4ad 新增第三方netcatty终端使用scoop install netcatty安装
* 86079a8 progress.md 更新提交历史
* 1d895be 六款带版本名 exe 统一改造：pre_install 通配符改名 + 固定 bin/shortcuts（升级后文件名与命令名不再随版本变化）
* c5f60cc progress.md 更新提交历史
...
* c41a1dc 更新 .gitignore：忽略 __pycache__ 与 *.pyc
* 42d91d6 progress.md 更新提交历史
...
* a7d0cc1 自动更新第三方软件
* af6aa24 progress.md 更新提交历史
* 2417bf8 SwitchHosts 更新便携包 hash（用户修改版）
* 390adc5 progress.md 更新提交历史
* 735a6ae 智能模式工具箱：文件名预选自动执行 + README 使用说明
* bb24f35 progress.md 更新提交历史
...
* a1da3ee DropIt 更新：重新上传扁平结构便携包并更新 hash
* b5e2aee 第三方更新
